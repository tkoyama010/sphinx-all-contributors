...
def test_all_contributors_directive_with_emoji(
    ...
    # Note: emojis should be: code=💻, doc=📖, design=🎨, test=⚠️
    assert list_items[0].astext() == "John Doe for 💻 code, 📖 doc", (  # noqa: S101
        "First list item text should include emojis"
    )
    assert list_items[1].astext() == "Jane Smith for 🎨 design, ⚠️ test", (  # noqa: S101
...
def test_all_contributors_directive_emoji_with_unknown_type(
    ...
    assert list_items[0].astext() == "John Doe for 💻 code, unknown_type", (  # noqa: S101
